...
func (v Vec3) Mul(n double) *Vec3   { return NewVec3(v.X*n, v.Y*n, v.Z*n) }
func (v Vec3) MulVec(x *Vec3) *Vec3 { return NewVec3(v.X*x.X, v.Y*x.Y, v.Z*x.Z) }
func (v Vec3) Div(n double) *Vec3   { return NewVec3(v.X/n, v.Y/n, v.Z/n) }
...
func (v Vec3) Refract(n *Vec3, etaIOverEtaT double) *Vec3 {
	...
	rOutParallel := v.Add(n.Mul(cosTheta)).Mul_(etaIOverEtaT)
	rOutPerp := n.Mul(-math.Sqrt(1.0 - rOutParallel.LengthSquared()))
	return rOutParallel.Add_(rOutPerp)
}
